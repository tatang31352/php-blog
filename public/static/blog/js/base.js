 ...
 $('#article_search').keydown(function(event) {
  ...
  /*  element.on('nav(demo)', function(elem){
    ...
    $('.message_submit').on('click',function () {
      ...
      //   layer.alert(content);
       // return false;
        ...
        $.post('message/add',{content:content,param:param,_token:token},function (res) {
           res = JSON.parse(res)
            if(res.code==1){
    ...
    $('.article_common_submit').on('click',function () {
        ...
        $.post('article/messageAdd',{content:content,article_id:id,param:param,_token:token},function (res) {
            res = JSON.parse(res)
            if(res.code==1){
